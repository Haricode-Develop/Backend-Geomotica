...
function insert_fertilizacion_data {
    local csv_file=$1
    ...
    echo "INSERT INTO fertilizacion (LONGITUD, LATITUD, CULTIVO, PARCELA, NOMBRE_FINCA, CODIGO_FINCA, AREA_BRUTA_Ha, AREA_NETA_Ha, DIFERENCIA_DE_AREA_Ha, FRANJA_DE_APLICACION_m, RESPONSABLE, ACTIVIDAD, EQUIPO, CODIGO_DE_MAQUINA, OPERADOR, FECHA_INICIO, FECHA_FINAL, HORA_INICIO, HORA_FINAL, TIEMPO_TOTAL_H, EFICIENCIA_Hora_Ha, PRODUCTO_APLICADO, DOSIS_TEORICA_Kg_ha, DOSIS_REAL_Kg_ha, VELOCIDAD_Km_H, ALTURA_GPS_m, TCH, TAH, ID_ANALISIS) VALUES" > temp.sql
    ...
    if ! mysql -u $DB_USER -p$DB_PASS $DB_NAME < temp.sql; then
        echo "Error al insertar en la tabla Fertilización"
        rm temp.sql
        exit 1
    fi
...
CSV_FILE="${PARENT_DIR}/FERTILIZACION.csv"
dos2unix $CSV_FILE

# Pasar el ID_ANALISIS_TIPO_RESULT como segundo argumento a insert_aps_data
insert_fertilizacion_data $CSV_FILE $ID_ANALISIS_TIPO_RESULT
