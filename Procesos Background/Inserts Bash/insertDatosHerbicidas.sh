...
function insert_herbicidas_data {
    local csv_file=$1
    ...
    echo "INSERT INTO herbicidas (LONGITUD, LATITUD, CULTIVO, PARCELA, NOMBRE_FINCA, CODIGO_FINCA, AREA_BRUTA_Ha, AREA_NETA_Ha, DIFERENCIA_DE_AREA_Ha, CODIGO_DE_MAQUINA, RESPONSABLE, ACTIVIDAD, EQUIPO, OPERADOR, FECHA_INICIO, FECHA_FINAL, HORA_INICIO, HORA_FINAL, TIEMPO_TOTAL_H, EFICIENCIA_Hora_Ha, VELOCIDAD_Km_H, TCH, TAH, ID_ANALISIS) VALUES" > temp.sql
    ...
    awk -F',' -v id_tipo=$id_analisis_tipo '
        ...
        NR > 1 {
            ...
            echo "Datos a insertar: LONGITUD = ${$1}, LATITUD = ${$2}, CULTIVO = ${$3}, PARCELA = ${$4}, NOMBRE_FINCA = ${$5}, CODIGO_FINCA = ${$6}, AREA_BRUTA = ${$7}, AREA_NETA = ${$8}, DIFERENCIA_DE_AREA = ${$9}, CODIGO_DE_MAQUINA = ${$10}, RESPONSABLE = ${$11}, ACTIVIDAD = ${$12}, EQUIPO = ${$13}, OPERADOR = ${$14}, FECHA_INICIO = ${fecha_inicio}, FECHA_FINAL = ${fecha_final}, HORA_INICIO = ${$17}, HORA_FINAL = ${$18}, TIEMPO_TOTAL = ${$19}, EFICIENCIA = ${$20}, VELOCIDAD_Km_H = ${$21}, TCH = ${$22}, TAH = ${$23}, ID_ANALISIS = ${id_tipo}" > "/dev/stderr"
    ...
    if ! mysql -u $DB_USER -p$DB_PASS $DB_NAME < temp.sql; then
        echo "Error al insertar en la tabla Herbicidas"
        rm temp.sql
        exit 1
    fi
...
insert_herbicidas_data $CSV_FILE $ID_ANALISIS_TIPO_RESULT
